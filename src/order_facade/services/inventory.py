...
class InventoryService:
    ...
    def reserve(self, sku: str, qty: int) -> bool:
        ...
        if self.check_stock(sku, qty):
            self._stock[sku] -= qty
            print(f"[Inventory] Reservados {qty} unidades de {sku}. Stock restante: {self._stock[sku]}")
            return True
        print(f"[Inventory] No se pudo reservar {qty} unidades de {sku}. Stock disponible: {self._stock.get(sku, 0)}")
        return False
    ...
    def release(self, sku: str, qty: int) -> None:
        ...
        print(f"[Inventory] Liberados {qty} unidades de {sku}. Stock actual: {self._stock[sku]}")
