...
class PaymentGateway:
    ...
    def charge(self, payment_info: Dict, amount: float) -> PaymentReceipt:
        ...
        if behavior in ["visa_success", "mastercard_success"]:
            ...
            print(f"[Payment] Cargo exitoso: ${amount:.2f} en tarjeta {card_type} ****{card_number[-4:]}")
            return PaymentReceipt(
                success=True, 
    ...
    def refund(self, transaction_id: str, amount: float) -> PaymentReceipt:
        ...
        if not transaction_id:
            return PaymentReceipt(
                success=False,
                message="ID de transacción requerido para reembolso"
            )
        
        refund_id = str(uuid.uuid4())
        print(f"[Payment] Reembolso procesado: ${amount:.2f} (TX: {transaction_id[:8]}...)")
        return PaymentReceipt(
            success=True,
