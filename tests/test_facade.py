...
class MockPaymentGateway(PaymentGateway):
    ...
    def charge(self, payment_info, amount):
        ...
        return PaymentReceipt(success=False, message="Test payment declined")
...
class MockShippingService(ShippingService):
    ...
    def create_shipment(
        self, customer_id, items, shipping_address=None, shipping_type="standard"
    ):
        self.created_shipments.append(
            {"customer_id": customer_id, "items": items, "shipping_type": shipping_type}
        )
        ...
        return ShipmentInfo(success=False, message="Test shipping failed")
...
class MockNotificationService(NotificationService):
    ...
    def notify(self, customer_id, message, channel=NotificationChannel.EMAIL):
        self.sent_messages.append(
            {"customer_id": customer_id, "message": message, "channel": channel}
        )
        return True

    def send_order_notification(
        self, customer_id, notification_type, order_data, channels=None
    ):
        self.sent_messages.append(
            {
...
class TestNotificationService:
    ...
    def test_order_notification_templates(self):
        ...
        result = notifications.send_order_notification(
            "customer-123", "order_confirmed", order_data
        )
...
class TestOrderFacade:
    ...
    def test_place_order_payment_declined(self):
        ...
        facade = OrderFacade(
            inventory=inventory, payments=payments, notifications=notifications
        )
        ...
        payment_fail_notifications = [
            msg
            for msg in notifications.sent_messages
...
class TestIntegration:
    ...
    def test_order_workflow_with_different_shipping_types(self):
        ...
        result_standard = facade.place_order(
            "customer-1", "TABLET-10", 1, payment_info, 300.0, shipping_type="standard"
        )
